...
import { parseMovieDetail, parseDownloadPage, parseDirectDownloadPage, parseDirectFileHost } from './detail.parser';
...
export class ScraperError extends Error {
  ...
  constructor(type: string, message: string, retryable: boolean, statusCode?: number, url?: string) {
    super(message);
...
export class ScraperService {
  ...
  public registerWebViewTrigger(
    trigger: (
      request: ScraperSessionRequest,
      onCompleted: (result: ScraperSessionResult | null, error?: string) => void
    ) => void
  ) {
  ...
  private runWebViewScrapeSession(
    ...
    if (!this.webViewTrigger) {
      return Promise.reject(new Error('WebView crawler trigger is not registered. Cannot scrape dynamic sites.'));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.log(`Initiating dynamic multi-stage scrape session for: ${url}`, 'info');

      this.webViewTrigger!(request, (result, error) => {
        if (error) {
          try {
            const parsed = JSON.parse(error);
            if (parsed && (parsed.type === 'PORTAL_ACCESS_DENIED' || parsed.type === 'WEBVIEW_HTTP_ERROR')) {
          ...
          this.log(`Scraper session succeeded in ${result.duration}ms. Hops: ${result.redirectCount}, Clicks: ${result.clickCount}`, 'success');
          resolve(result);
        } else {
   ...
   */
  public async detectStack(url: string): Promise<{ stack: string; isStatic: boolean }> {
    ...
    try {
      ...
      if (isWordpress) {
        this.log(`Detected tech: WordPress (Static / Server-Rendered HTML)`, 'success');
        return { stack: 'WordPress', isStatic: true };
      }

      // Check Next.js
      if (html.includes('__NEXT_DATA__') || html.includes('/_next/')) {
        this.log(`Detected tech: Next.js (Server-Rendered & Rehydrated)`, 'success');
      ...
      const hasAppRoot = html.includes('<app-root') || html.includes('id="root"') || html.includes('id="app"');
      const isThinShell = html.length < 5000 && (html.includes('bundle.js') || html.includes('/static/js/'));

      if (hasAppRoot && isThinShell) {
        this.log(`Detected tech: Single Page Application (React/Angular client-rendered)`, 'warn');
      ...
      this.log(`Network check failed during tech detection: ${err.message}. Defaulting to dynamic client.`, 'warn');
   ...
   */
  private async fetchHtmlWithRetry(url: string, retries = 3, delay = 2000): Promise<string> {
    const userAgent = USER_AGENTS[Math.floor(Math.random() * USER_AGENTS.length)];
    
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        this.log(`Fetching HTML (Attempt ${attempt}/${retries}) for: ${url.substring(0, 50)}...`, 'info');
        const response = await axios.get(url, {
          headers: {
        ...
        if (statusCode === 401 || statusCode === 403) {
          this.log(`Portal rejected automated request: HTTP ${statusCode}`, 'warn');
  ...
  public isDirectFileUrl(url: string): boolean {
    ...
    if (
      ...
      (lowerUrl.includes('filename') && (lowerUrl.includes('.zip') || lowerUrl.includes('.mkv') || lowerUrl.includes('.mp4')))
    ) {
      return true;
    }

    // 3. Check regular extensions at the end of path or before query parameters
    const fileExtensionRegex = /\.(mkv|mp4|avi|zip|rar|tar|gz|mov|wmv|3gp|7z|dmg|iso|mp3|m4a|epub|pdf)(\?|#|$)/i;
   ...
   */
  public async scrapeCatalogPage(url: string, forceDynamic = false): Promise<{ items: CatalogItem[]; nextPageUrl: string | null }> {
    let html = '';
    ...
    if (forceDynamic) {
      ...
      if (detect.isStatic) {
        ...
        this.log(`Tech stack suggests dynamic loading. Launching WebView engine...`, 'warn');
        html = await this.fetchHtmlViaWebView(url);
      }
    }
    ...
    this.log(`Extracted ${result.items.length} items from listing page.`, 'success');
    return result;
  }
   ...
   */
  public async scrapeMovieDetail(url: string, forceDynamic = false): Promise<MovieDetail> {
    let html = '';
    ...
    if (forceDynamic) {
      ...
      if (detect.isStatic) {
        ...
        this.log(`Tech stack suggests dynamic details. Launching WebView engine...`, 'warn');
        html = await this.fetchHtmlViaWebView(url);
      }
    }
   ...
   */
  public async scrapeDownloadPage(url: string, forceDynamic = false): Promise<{ label: string; url: string }[]> {
    if (this.isDirectFileUrl(url)) {
      this.log(`Instant Resolution: URL is already a direct file link: ${url.substring(0, 45)}...`, 'success');
    ...
    if (forceDynamic) {
      try {
        ...
        if (webViewErr && webViewErr.type === 'PORTAL_ACCESS_DENIED') {
          this.log(`[Scraper] Portal rejected automated request: HTTP ${webViewErr.statusCode || 403}`, 'error');
      ...
      try {
        ...
        if (err && err.type === 'PORTAL_ACCESS_DENIED') {
          ...
          this.log(`Axios portal load failed, falling back to WebView: ${err.message}`, 'warn');
        }
    ...
    if (mirrors.length === 0 && !forceDynamic) {
      this.log(`No mirror links found on static HTML. Retrying with full browser WebView engine...`, 'warn');
      try {
        const session = await this.runWebViewScrapeSession(url, 'mirrors');
        html = session.html;
        finalUrl = session.finalUrl;
        mirrors = parseDownloadPage(html, finalUrl);
        if (mirrors.length === 0 && session.mirrors && session.mirrors.length > 0) {
          mirrors = session.mirrors;
        }
      } catch (err: any) {
        if (err && err.type === 'PORTAL_ACCESS_DENIED') {
          this.log(`[Scraper] Portal rejected automated request: HTTP ${err.statusCode || 403}`, 'error');
          this.log(`[Scraper] Automated portal access unavailable`, 'warn');
          this.log(`[Scraper] Interactive browser flow required`, 'info');
          this.log(`[Scraper] Closing WebView scraper session cleanly`, 'info');
          throw new ScraperError(
   ...
   */
  public async scrapeDirectDownloadPage(url: string, forceDynamic = false): Promise<{ label: string; url: string }[]> {
    if (this.isDirectFileUrl(url)) {
      this.log(`Instant Resolution: URL is already a direct file link: ${url.substring(0, 45)}...`, 'success');
      return [{ label: 'Direct Download', url }];
    }
    ...
    if (forceDynamic) {
      ...
      try {
        ...
        if (err && err.type === 'PORTAL_ACCESS_DENIED') {
          ...
          this.log(`Axios direct file fetch failed, trying WebView: ${err.message}`, 'warn');
        }
        try {
          const session = await this.runWebViewScrapeSession(url, 'direct-links');
          html = session.html;
    ...
    if (directLinks.length === 0 && !forceDynamic) {
      this.log(`No direct links found on static HTML. Retrying with full browser WebView engine...`, 'warn');
      try {
        const session = await this.runWebViewScrapeSession(url, 'direct-links');
        html = session.html;
        finalUrl = session.finalUrl;
        directLinks = parseDirectDownloadPage(html, finalUrl);
        if (directLinks.length === 0 && session.mirrors && session.mirrors.length > 0) {
        ...
        this.log(`WebView fallback direct parse failed: ${err.message}`, 'error');
      }
    }
   ...
   */
  public async scrapeDirectFileHost(url: string, forceDynamic = false): Promise<string | null> {
    if (this.isDirectFileUrl(url)) {
      this.log(`Instant Resolution: URL is already a direct file link: ${url.substring(0, 45)}...`, 'success');
      return url;
    }
    ...
    if (url.includes('pixeldrain.com') || url.includes('pixeldrain.dev')) {
      const match = url.match(/(?:pixeldrain\.com|pixeldrain\.dev)\/u\/([a-zA-Z0-9_-]+)/);
      if (match) {
    ...
    if (forceDynamic) {
      ...
      try {
        ...
        if (err && err.type === 'PORTAL_ACCESS_DENIED') {
          ...
          this.log(`Axios file host fetch failed, trying WebView: ${err.message}`, 'warn');
        }
        try {
          const session = await this.runWebViewScrapeSession(url, 'direct-file');
          html = session.html;
    ...
    if (!directLink && !forceDynamic) {
      this.log(`No direct links parsed on host static HTML. Retrying with WebView engine...`, 'warn');
      try {
        const session = await this.runWebViewScrapeSession(url, 'direct-file');
        html = session.html;
        finalUrl = session.finalUrl;
